...
async function makeApiCall(endpoint, data, cacheKey) {
  ...
  const request = fetch(url, {
    ...
    .then(async (response) => {
      ...
      if (!response.ok || responseData.responseStatus?.code !== API_CONFIG.SUCCESS_CODE) {
        throw new Error(responseData.responseStatus?.message || 'Unknown error');
      }
...
export async function bookSlotForDay(bookingData) {
  ...
  if (appointmentType === 'vaccination' && (!cartItems || cartItems.length === 0)) {
  ...
  const finalServiceName = appointmentType === 'consultation' 
    ? getConsultancyDisplayName(consultancyType)
    : (vaccineNames || (cartItems && cartItems.map(item => item.name).join(',')) || '');
  ...
  const payload = {
    ...
    vaccinesIDs: finalServiceName, // Using the same field for both vaccine and consultancy names
...
export async function fetchConsultancyTypes() {
  try {
    ...
    if (responseData.responseStatus?.code === 1001) {
      ...
      return {
        success: false,
        message: responseData.responseStatus?.message || 'Failed to fetch consultancy types',
